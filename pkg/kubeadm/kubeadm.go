...
func GenerateTemplates(log logr.Logger, clusterName string) (map[string]string, error) {
	...
	if err := os.MkdirAll("/tmp/"+clusterName, 0755); err != nil {
	...
	localKASSubcommand := append(KASSubcommand, "/tmp/"+clusterName+DefaultKubeadmConfigPath, "--rootfs", "/tmp/"+clusterName)
	KCMSubcommand = append(KCMSubcommand, "--rootfs", "/tmp/"+clusterName)
	EtcdSubcommand = append(EtcdSubcommand, "--rootfs", "/tmp/"+clusterName)
	// generate the manifests
	if err := execCommand(log, KubeadmExecPath, localKASSubcommand...); err != nil {
...
func generateKubeadmConfig(clusterName string) error {
	...
	return os.WriteFile("/tmp/"+clusterName+DefaultKubeadmConfigPath, []byte(completedKubeadmConfig), 0600)
}
