...
func (s *Service) Reconcile(ctx context.Context) (res reconcile.Result, err error) {
	...
	kubeAPIServerPodSpec := podSpecs[kubeadmconstants.KubeScheduler]
	kubeAPIServerPodSpec.Name = fmt.Sprintf("%s-%s", s.scope.K8sControlPlane.Name, kubeadmconstants.KubeScheduler)
...
func (s *Service) createOrUpdateApiServerCertificates(ctx context.Context) error {
	controlPlaneSecretName := fmt.Sprintf("%s-scheduler-kubeconfig", s.scope.Cluster.Name)
	...
	schedulerSecret.Data[kubeadmconstants.SchedulerKubeConfigFileName] = configSecret.Data[secret.KubeconfigDataName]
	schedulerSecret.SetOwnerReferences([]metav1.OwnerReference{*metav1.NewControllerRef(s.scope.K8sControlPlane, controlplanev1.GroupVersion.WithKind("K8sControlPlane"))})
	err = s.scope.ManagerClient.Create(ctx, schedulerSecret)
	if err != nil {
		return err
	}
